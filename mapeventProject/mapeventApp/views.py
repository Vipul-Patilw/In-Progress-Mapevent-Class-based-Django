...
from mapeventApp.models import  Event, Login,ForgotPassword,Staff,Add,LocateEvent
...
def addevent(request):
	...
	if request.method =="POST":
		...
		maping = AddEvent(event=event,info=info,lang=lang,lat=lat,eventaddress=eventaddress,fromdate=fromdate,fromtime=fromtime,todate=todate,totime=totime,icon=icon, location=location)
		maping.save()
...
def map(request):
	...
	if request.method =="POST":
		...
		maps = LocateEvent(lat=lat,lang=lang)
		maps.save()
		maping = LocateEvent.objects.filter(lat=lat).all()
		return  render(request,'map.html',{'maps':maping})	
